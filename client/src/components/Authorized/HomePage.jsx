...
function Home() {
  ...
  return (
    <div id="page" className="w-full pt-16 scale-100">
      <div id="rectangle" className="flex items-center h-2/3 mx-30 mt-20 bg-white">
        <div id="hero-text" className="w-1/2 flex justify-center pl-15">
          <div className="flex flex-col md:block "> 
            {/* we are checking if the user is logged in to decide what hero text to display */}
            {authState.authDone ? (
              <h1 className="text-5xl text-autumn-leaves-4">
                Welcome back, {authState.userDets?.UserName}
              </h1>
            ) : (
              <h1 data-testid="vitest-hero-text" className="text-5xl text-autumn-leaves-4">Welcome to Recipe HUB.</h1>
            )}
        ...
        <div id="hero-image" className="w-1/2 flex justify-center">
          <img
      ...
      <div id="card-holder" className="bg-autumn-leaves-1 py-10 px-60 mt-20">
        <div className="flex flex-col items-center">
          ...
          <h1 className="text-autumn-leaves-4 motion-preset-rebound-up text-lg transition-opacity ease-in duration-200 opacity-100 mt-2">
            click on know more on a recipe card to find the instructions,
            ingredients and, more.
          </h1>
          <button onClick={() => authState.authDone ? refetch() : navigate('/login')} 
          className="bg-white rounded-2xl p-5 mt-5 text-xl text-autumn-leaves-3 cursor-pointer transition duration-150 
        ...
        {isLoading ? (
          ...
          <div className="mt-10 flex justify-between flex-wrap p-10 rounded-xl">
            {data?.map((item, index) => {
